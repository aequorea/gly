#!/usr/bin/python
# gly15.py -- protein glycosylator
# 2016.11.02 -- John Saeger
...
common_sheet_glyco_sites = set()
common_helix_glyco_sites = set()
common_coil_glyco_sites = set()
for chain in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
	...
	sheet_glyco_sites, sheet_short = get_glyco_sites(sheet_score, sequence, pairs, sheet)
	print("\nChain {} has {} potential beta sheet glycosylation sites:".format(chain, len(sheet_glyco_sites)))
	print("[site, score, seq, delta hydropathy]")
	for gs in sheet_glyco_sites: print gs

	if len(common_sheet_glyco_sites) == 0:
		common_sheet_glyco_sites.update(sheet_short)
	else:
		new_sheet_glyco_sites = set(sheet_short)
		common_sheet_glyco_sites = common_sheet_glyco_sites & new_sheet_glyco_sites
	
	helix_glyco_sites, helix_short = get_glyco_sites(helix_score, sequence, pairs, helix)
	print("\nChain {} has {} potential alpha helix glycosylation sites:".format(chain, len(helix_glyco_sites)))
	print("[site, score, seq, delta hydropathy]")
	for gs in helix_glyco_sites: print gs

	if len(common_helix_glyco_sites) == 0:
		common_helix_glyco_sites.update(helix_short)
	else:
		new_helix_glyco_sites = set(helix_short)
		common_helix_glyco_sites = common_helix_glyco_sites & new_helix_glyco_sites
	
	coil_glyco_sites, coil_short = get_glyco_sites(helix_score, sequence, pairs, coil)
	print("\nChain {} has {} potential random coil glycosylation sites:".format(chain, len(coil_glyco_sites)))
	print("[site, score, seq, delta hydropathy]")
	for gs in coil_glyco_sites: print gs

	if len(common_coil_glyco_sites) == 0:
		common_coil_glyco_sites.update(coil_short)
	else:
		new_coil_glyco_sites = set(coil_short)
		common_coil_glyco_sites = common_coil_glyco_sites & new_coil_glyco_sites
	
print("\nThere are {} common potential beta sheet glycosylation sites:".format(len(common_sheet_glyco_sites)))
print(list(common_sheet_glyco_sites))

print("\nThere are {} common potential alpha helix glycosylation sites:".format(len(common_helix_glyco_sites)))
print(list(common_helix_glyco_sites))

print("\nThere are {} common potential random coil glycosylation sites:".format(len(common_coil_glyco_sites)))
print(list(common_coil_glyco_sites))
